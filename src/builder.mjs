...
let catalog = []
let selected = new Map() // name -> {id,name}
let activeCat = ''

async function fetchCatalog() {
  const dshDir = dshDirEl.value.trim() || '../../deepseek-harness'
  logEl.textContent = `加载插件目录… ${dshDir}`
  try {
    catalog = await window.builderAPI.listPlugins(dshDir)
    const ver = await window.builderAPI.getDshVersion(dshDir)
    if (ver) verBadge.textContent = `DSH ${ver}`
  } catch (e) {
    logEl.textContent = '加载失败: ' + e.message
    return
  }
  if (!catalog.length) {
    logEl.textContent = '未发现插件 — 检查 DSH_DIR 与 dsh-hub/plugins 是否已 submodule init'
  } else {
    const uq = catalog.filter(c=>c.unique).length
    logEl.textContent = `已加载 ${catalog.length} 个插件（唯一 ${uq} / 不限数 ${catalog.length-uq}）— 勾选后一键出包`
  }
  renderCats()
  render()
}
...
function renderCats() {
  ...
  catListEl.innerHTML = `<div class="cat-item ${!activeCat?'active':''}" data-cat="">全部 <span class="n">${catalog.length}</span></div>` +
    cats.map(c=>`<div class="cat-item ${activeCat===c?'active':''}" data-cat="${c}">${c} <span class="n">${catalog.filter(x=>x.category===c).length}</span></div>`).join('')
...
function linkFor(p) {
  ...
  if (n.startsWith('dsh-')) return `https://github.com/Lin-A1/dsh-hub/tree/main/plugins/${p.category}/${n}`
  return null
}
...
function render() {
  ...
  for (const p of catalog) {
    ...
    row.innerHTML = `
      ...
      <div class="pinfo">
        <div class="pname">${p.name} ${tag} <a href="${href||'#'}" target="_blank" onclick="event.preventDefault();${href?`window.builderAPI.openExternal('${href}')`:''}">${href?'↗':''}</a></div>
    ...
    cb.onchange = () => {
      if (cb.checked) {
        // unique constraint: same category unique plugins are mutually exclusive
        if (p.unique) {
  ...
  if (!shown) listEl.innerHTML = `<div class="empty">${onlySel?'尚未勾选任何插件':(q?'无匹配结果':'暂无插件 — 请检查 DSH_DIR 或添加自定义源')}</div>`
}
...
window.build = async () => {
  ...
  logEl.textContent = `[build] mode=${mode} product=${productName} 插件 ${plugins.length}\n` + JSON.stringify(plugins,null,2)
  try {
